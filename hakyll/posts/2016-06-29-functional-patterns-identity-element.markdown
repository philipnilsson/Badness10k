...
The pattern **Identity element** is all about defining the concept of
...
you're working on right now --- values like the numbers `1` and `0`, the empty string `""`
or the empty array `[]`, that are often used as initializers and
somehow feel "empty" or uninteresting.
